...
// HashRing holds the information about the members of the hashring hash circle.
...
// Add inserts a string element in the hashring hash.
...
func (c *HashRing) GetTwo(name string) (string, string, error) {
	c.RLock()
	defer c.RUnlock()
	if len(c.circle) == 0 {
		return "", "", ErrEmptyCircle
